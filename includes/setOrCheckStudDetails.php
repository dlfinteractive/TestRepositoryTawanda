  ...
  session_start();
   ...
   include ('connections.php');
	  ...
	  //insert student details
		...
		$statement="merge into MAFMARKET_STUDENT using dual on 
			(STUDENT_NUMBER=:student_number)
		 when matched then 
		     UPDATE SET STUDENT_FNAME = :student_fname, STUDENT_LNAME = :student_lname 
		 when not matched then 
		    INSERT (STUDENT_FNAME,STUDENT_LNAME,STUDENT_NUMBER) values (:student_fname,:student_lname,:student_number)";		
		...
		if (ociexecute($s1) === FALSE):	
		    ...
		    $message = $student_fname.'*'.$student_lname.'*'.$student_number;
