  ...
  session_start();
   ...
   include ('connections.php');
	  ...
	  //insert student details
		...
		$participation = trim($_POST['PostPARTICIPATION']);

		$statement="merge into MAFMARKET_STUDENT_SCORES using dual on 
			(STUDENT_ID=:student_id)
		 when matched then 
		     UPDATE SET PARTICIPATION = :participation 
		 when not matched then 
		    INSERT (PARTICIPATION) values (:participation)";		
		
		$s1 = ociparse($conn, $statement);
		ociBindByName($s1,':student_id',$student_id,-1); 
		ociBindByName($s1,':participation',$participation,-1);
